...
export default function LoginPage() {
  ...
  // 1. Check for stored login errors immediately upon page mount/remount
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedError = sessionStorage.getItem('login_error');
      if (savedError) {
        setError(savedError);
        sessionStorage.removeItem('login_error'); // Clear it so it doesn't stay forever
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const targetMessage = err.response?.data?.message || 'Login failed. Please try again.';
      
      // 2. Persist the error to sessionStorage right before unmount triggers
      sessionStorage.setItem('login_error', targetMessage);
  ...
  const handleOtpSubmit = async (otp) => {
    ...
    try {
      ...
      const targetMessage = err.response?.data?.message || 'OTP verification failed.';
      sessionStorage.setItem('login_error', targetMessage);
      setError(targetMessage);
    } finally {
      setLoading(false);
