...
export default function IncomeForm({ income, onClose, users = null, defaultUserId = '' }) {
  ...
  const [form, setForm] = useState({
    ...
    date: income?.date ? new Date(income.date).toISOString().split('T')[0] : today,
    userId: income?.user?._id ?? income?.user ?? defaultUserId,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const payload = { amount: form.amount, source: form.source, description: form.description, date: form.date };
