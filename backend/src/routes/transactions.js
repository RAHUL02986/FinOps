...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // IMPORTANT: Only apply the balance impact after approval
      if (txn.status === 'Approved' && txn.account) {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const approvedBefore = oldStatus === 'Approved';
    const approvedAfter = nextStatus === 'Approved';

    // Apply/reverse only when approval status changes.
    // (If status is unchanged and still not Approved, we must not touch balances.)
    if (approvedBefore !== approvedAfter) {
      // Reverse effect if it was previously approved
      if (approvedBefore && oldAccount) {
      ...
      // Apply effect if it becomes approved
      if (approvedAfter && updated.account) {
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Reverse account balance ONLY if the transaction was approved
    if (txn.status === 'Approved' && txn.account) {
      const Account = require('../models/Account');
      const account = await Account.findById(txn.account);
      if (account) {
