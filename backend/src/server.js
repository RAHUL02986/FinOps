...
// Import database config and models needed for socket handlers
const connectDB = require('./config/database');
const User = require('./models/User'); // Ensure this points to your actual User model path
// const ChatMessage = require('./models/ChatMessage'); // Ensure this points to your actual ChatMessage model path
...
// DYNAMIC CORS SOLUTION: Allows localhost and local network IPs (e.g., 192.168.x.x) to fetch data
app.use(
  cors({
    origin: (origin, callback) => {
      if (!origin) return callback(null, true); // Allow non-browser requests (Postman, mobile setups)
      
      if (process.env.FRONTEND_URL && origin === process.env.FRONTEND_URL) {
        return callback(null, true);
      }

      // Regex matching localhost configurations or common LAN IP spaces (192.168.x.x or 10.x.x.x)
      const isLocalNetwork = /^(https?:\/\/localhost(:[0-9]+)?)|(https?:\/\/192\.168\.[0-9.]+(:[0-9]+)?)|(https?:\/\/10\.[0-9.]+(:[0-9]+)?)$/.test(origin);
...
// Socket.io Chat Server setup with network allowance
const server = http.createServer(app);
const io = new Server(server, {
  cors: {
    // Allows any local origin handshaking during development setups
...
// Final Setup configuration to explicitly run on all network adapters
const PORT = process.env.PORT || 5000;
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Server + Socket.IO securely running on port ${PORT} (Network Exposed)`);
});